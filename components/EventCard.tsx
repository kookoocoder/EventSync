      ...
      <div className="aspect-video relative overflow-hidden">
        ...
        {event.event_type && (
          <div className="absolute bottom-3 left-3 z-10">
            <span className="inline-block bg-black/50 backdrop-blur-sm text-white text-xs px-2 py-1 rounded-full mr-2 capitalize">
              {event.event_type}
            </span>
          </div>
        )}
      ...
      <div className="flex flex-col flex-1">
        ...
        <CardFooter className="px-4 py-3 mt-auto">
          {isPast || (event.end_date && new Date(event.end_date) < new Date()) ? (
             <Link href={`/${eventType}/${event.id}/results`} className="w-full"> {/* Assuming a results page */}

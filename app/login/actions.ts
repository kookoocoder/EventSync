    // EventSync/app/login/actions.ts
    'use server';
    ...
    export async function login(
      ...
      if (!email || !password) {
        // console.log("[Login Action] Email or password missing."); // Commented out
        return { error: "Email and password are required." };
      }

      // console.log(`[Login Action] Attempting login for: ${email}`); // Commented out
      ...
      if (error) {
        console.error(`[Login Action] Login failed for ${email}:`, error.message); // Keep error log
        return { error: `Login failed: ${error.message}` }; // Return error state
      }

      if (!data.user) {
          console.error(`[Login Action] Login successful but no user data returned for ${email}.`); // Keep error log
          return { error: 'Login succeeded but failed to retrieve user data.' }; // Return error state
      }

      // console.log(`[Login Action] Login successful for user: ${data.user.id}, email: ${data.user.email}`); // Commented out
      ...
      // console.log(`[Login Action] Redirecting to: ${redirectPath}`); // Commented out

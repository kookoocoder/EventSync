    ...
    const eventSchema = z.object({
        ...
        message: "Cross-field validation failed.", // Generic message for refine error
    });
    ...
    async function uploadFile(
        ...
        if (uploadError) {
            console.error(`[Upload Helper] Supabase Upload Error (${bucket}):`, uploadError); // Keep error log
        ...
        if (!urlData?.publicUrl) {
            console.error(`[Upload Helper] Failed to get public URL for ${filePath}`); // Keep error log
            await supabaseService.storage.from(bucket).remove([filePath]); // Attempt cleanup
            throw new Error(`Failed to retrieve public URL after upload for ${bucket}.`);
        }
        // console.log(`[Upload Helper] File uploaded to ${bucket}: ${urlData.publicUrl}`); // Commented out
    ...
    async function cleanupFile(
        ...
        try {
            ...
            if (filePath) {
                 // console.log(`[Cleanup Helper] Attempting to remove file: ${filePath} from ${bucket}`); // Commented out
                 const { error: removeError } = await supabaseService.storage.from(bucket).remove([filePath]);
                 if (removeError) {
                     console.error(`[Cleanup Helper] Failed to remove file ${filePath} from ${bucket}:`, removeError); // Keep error log
                 } else {
                     // console.log(`[Cleanup Helper] Successfully removed file ${filePath} from ${bucket}`); // Commented out
                 }
            }
        } catch (e) {
             console.error(`[Cleanup Helper] Error parsing or removing file URL ${fileUrl}:`, e); // Keep error log
        }
    }

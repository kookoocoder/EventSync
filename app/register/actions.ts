    // EventSync/app/register/actions.ts
    'use server';
    ...
    export async function register(
        ...
        if (validationError) {
            // console.error("[Register Action] Validation failed:", validationError); // Keep or remove based on preference
            return { error: validationError };
        }

        // console.log(`[Register Action] Attempting registration for: ${data.email} as ${data.userType}`); // Commented out
        ...
        if (signUpError) {
            console.error("[Register Action] Supabase SignUp Error:", signUpError.message); // Keep error log
        ...
        if (!authData.user || !authData.user.id) {
            console.error("[Register Action] SignUp successful but no user data returned."); // Keep error log
            return { error: 'Registration partially succeeded, cannot create profile. Please contact support.' };
        }

        const userId = authData.user.id;
        // console.log(`[Register Action] Auth user created: ${userId}`); // Commented out
        ...
        try {
            ...
            // console.log(`[Register Action] Inserting into '${targetTable}' table for user: ${userId}`); // Commented out
            ...
            if (insertError) {
                 ...
                 if (insertError.code === '23505') { // Unique violation code
                    console.error(`[Register Action] Unique constraint violation inserting into ${targetTable}. User ID: ${userId}, Email: ${data.email}`); // Keep error log
                ...
                // console.log(`[Register Action] '${targetTable}' record created successfully for user: ${userId}`); // Commented out
            }

        } catch (error: any) {
            console.error(`[Register Action] Error inserting into '${targetTable}' table:`, error.message); // Keep error log
            dbInsertError = error;
        ...
        if (dbInsertError) {
           console.error(`[Register Action] Database profile creation failed for ${userId} (${data.userType}). Error: ${dbInsertError.message}`); // Keep error log
        ...
        // console.log(`[Register Action] Registration successful for ${data.email}. Redirecting to email confirmation page.`); // Commented out
